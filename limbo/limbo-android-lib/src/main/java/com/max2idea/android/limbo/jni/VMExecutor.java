...
private String getQemuLibrary() {
    ...
    /**
     * Maps the sound card names Limbo shows in its UI to QEMU device names.
     *
     * -soundhw was removed in QEMU 7.1; every card it used to cover is now
     * instantiated with -device. Note that intel-hda additionally needs a
     * codec device, and that "all" has no -device equivalent at all.
     */
    private static final Map<String, String[]> SOUNDHW_DEVICES;
    static {
        Map<String, String[]> m = new LinkedHashMap<>();
        m.put("sb16", new String[]{"sb16"});
        m.put("ac97", new String[]{"AC97"});
        m.put("adlib", new String[]{"adlib"});
        m.put("cs4231a", new String[]{"cs4231a"});
        m.put("gus", new String[]{"gus"});
        m.put("es1370", new String[]{"ES1370"});
        m.put("hda", new String[]{"intel-hda", "hda-duplex"});
        m.put("pcspk", new String[]{"isa-pcspk"});
        SOUNDHW_DEVICES = Collections.unmodifiableMap(m);
    }
    ...
    private void addAudioOptions(ArrayList<String> paramsList) {
        ...
        String[] devices = SOUNDHW_DEVICES.get(card.toLowerCase(Locale.ROOT));
        if (devices == null) {
            // "all" used to mean "every card at once"; there is no way to ask
            // for that with -device, and it was never a sensible thing to do
            // on a phone. Anything else is a stale value from an old install.
            Log.w(TAG, "Ignoring unsupported sound card '" + card
                    + "': -soundhw was removed in QEMU 7.1 and this name has "
    ...
    private void addGenericOptions(Context context, ArrayList<String> paramsList) {
        ...
        // -tb-size was removed in QEMU 6.0; the translation block cache size
        // is now a property of the tcg accelerator and is applied in
        // addAccelerationOptions().

        // -realtime mlock= was removed in QEMU 6.0 in favour of -overcommit.
    ...
    private void addCpuBoardOptions(ArrayList<String> paramsList) {
        ...
        // -no-acpi and -no-hpet were removed in QEMU 9.0 and turned into
        // machine properties, so they have to be folded into -machine rather
        // than passed separately.
        StringBuilder machineOpts = new StringBuilder();
        if (getMachineType() != null && !getMachineType().equals("Default")) {
            machineOpts.append(getMachineType());
        }
        if (getMachine().getDisableAcpi() != 0) {
            if (machineOpts.length() > 0) {
                machineOpts.append(",");
            }
            machineOpts.append("acpi=off");
        }
        if (getMachine().getDisableHPET() != 0) {
            if (machineOpts.length() > 0) {
                machineOpts.append(",");
            }
            machineOpts.append("hpet=off");
        }
        if (machineOpts.length() > 0) {
            paramsList.add("-machine");
            paramsList.add(machineOpts.toString());
        }
    ...
    private void addAccelerationOptions(ArrayList<String> paramsList) {
        ...
        if (getMachine().getEnableKVM() != 0) {
            ...
            // Replaces the -tb-size option removed in QEMU 6.0.
            if (Config.overrideTbSize && Config.tbSize != null) {
                tcgParams += ",tb-size=" + Config.tbSize;
            }
