...
JNIEXPORT jstring JNICALL Java_com_max2idea_android_limbo_jni_VMExecutor_start(
	...
	/*
	 * Entry points as of QEMU 11 (include/system/system.h):
	 *
	 *     void qemu_init(int argc, char **argv);
	 *     int  qemu_main_loop(void);
	 *     void qemu_cleanup(int status);
	 *
	 * qemu_init() lost its third (envp) parameter after QEMU 5.1, and
	 * qemu_main_loop()/qemu_cleanup() gained a status value. Calling them
	 * with the old signatures happened to work on ARM64 by accident; it is
	 * still undefined behaviour, so the prototypes below must be kept in
	 * sync with the QEMU release being bundled.
	 *
	 * QEMU's own main() in system/main.c does:
	 *
	 *     qemu_init(argc, argv);
	 *     bql_unlock(); replay_mutex_unlock();
	 *     ... then re-locks both and runs qemu_main_loop()
	 *
	 * On Linux the unlock/re-lock pair cancels out (the qemu_main hook is
	 * only non-NULL on Darwin), so calling the three functions back to back
	 * is equivalent -- and unlike main() it does not end in exit(), which
	 * would take the whole Android app down with it.
	 */
	typedef void (*qemu_init_t)(int argc, char **argv);
	typedef int (*qemu_main_loop_t)(void);
	typedef void (*qemu_cleanup_t)(int status);

	dlerror();
	qemu_init_t qemu_init = (qemu_init_t) dlsym(handle, "qemu_init");
	const char *dlsym_error = dlerror();
	if (dlsym_error) {
		LOGE("Cannot find qemu symbol 'qemu_init': %s\n", dlsym_error);
		dlclose(handle);
		handle = NULL;
		return (*env)->NewStringUTF(env, dlsym_error);
	}

	qemu_main_loop_t qemu_main_loop =
			(qemu_main_loop_t) dlsym(handle, "qemu_main_loop");
	dlsym_error = dlerror();
	if (dlsym_error) {
	...
	qemu_cleanup_t qemu_cleanup = (qemu_cleanup_t) dlsym(handle, "qemu_cleanup");
	dlsym_error = dlerror();
	if (dlsym_error) {
...
JNIEXPORT jstring JNICALL Java_com_max2idea_android_limbo_jni_VMExecutor_stop(
    ...
    if(restart_int) {
        ...
        qemu_system_reset_request(LIMBO_SHUTDOWN_CAUSE_GUEST_RESET);
        ...
        qemu_system_shutdown_request(LIMBO_SHUTDOWN_CAUSE_HOST_UI);
